...
export default function AIPortal({ isOpen, onClose }) {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      role: "assistant",
      text: "Stuck? I am your AI Idea Builder! Ask me for a prompt!",
    },
  ]);
  ...
  const handlePublish = () => {
    ...
    navigate("/add", {
      state: { generatedUrl: preview, generatedTitle: prompt },
    });
  };
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-white">
                <i className="ri-magic-line text-red-500 mr-2"></i> Inspira
                Magic Studio
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="w-full flex gap-2 mb-4">
              <input
                ...
                placeholder="e.g. A cyberpunk samurai city..."
                className="flex-1 bg-zinc-800 text-white px-5 py-3.5 rounded-xl border border-zinc-700 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all text-sm font-medium"
              />
              <button
                type="button"
                onClick={handleGenerateAi}
                disabled={loadingAi}
                className="bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg flex items-center justify-center cursor-pointer"
              >
                {loadingAi ? (
            ...
            <div className="flex-1 bg-zinc-800/80 rounded-2xl p-4 border border-zinc-700/50 flex flex-col h-[200px] shadow-inner">
              <div className="flex-1 overflow-y-auto space-y-3 pr-2 mb-3 custom-scrollbar">
                {chatMessages.map((msg, i) => (
                  ...
                  >
                    <div
                      className={`p-3 rounded-2xl max-w-[90%] text-sm ${msg.role === "user" ? "bg-red-600 text-white rounded-br-none" : "bg-zinc-700 text-gray-200 rounded-bl-none"}`}
                    >
                      <p>{msg.text}</p>
                      {msg.role === "assistant" && i !== 0 && (
                        <button
                          ...
                          className="mt-2 text-xs font-semibold bg-zinc-800 hover:bg-zinc-900 border border-zinc-600 px-3 py-1.5 rounded-full transition-colors flex items-center gap-1 active:scale-95 text-white"
                        >
              ...
              <form onSubmit={handleChatSubmit} className="relative mt-auto">
                <input
                  ...
                  placeholder="Ask AI for aesthetic ideas..."
                  className="w-full bg-zinc-900 text-white rounded-full py-2.5 pl-4 pr-10 text-sm border border-zinc-700 focus:outline-none focus:border-red-500"
                />
                <button
                  type="submit"
                  disabled={loadingChat || !chatInput.trim()}
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-red-600 hover:bg-red-700 disabled:bg-zinc-700 rounded-full text-white transition-colors cursor-pointer"
                >
            ...
            {history.length > 0 && (
              <div className="mt-4 bg-zinc-800/50 rounded-2xl p-4 border border-zinc-700/30">
                <p className="text-zinc-400 text-xs font-bold uppercase tracking-wider mb-3">
                  <i className="ri-history-line"></i> Session History
                </p>
          ...
          <div className="flex-1 bg-zinc-800 rounded-2xl flex flex-col items-center justify-center min-h-[300px] border border-zinc-700 relative overflow-hidden group w-full max-w-[400px]">
            ...
            {loadingAi && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-800 z-10 rounded-2xl">
                ...
                <p className="text-zinc-400 font-medium text-sm text-center px-4">
            ...
            {preview && !loadingAi && (
              <>
                <button
                  onClick={clearImage}
                  className="absolute top-4 right-4 bg-black/50 hover:bg-black/80 text-white p-2 rounded-full backdrop-blur-md transition-colors shadow-lg cursor-pointer z-20"
                >
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent flex justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20">
                  <button
                    ...
                    className="w-full max-w-[250px] bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-full shadow-xl transition-transform active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                  >
